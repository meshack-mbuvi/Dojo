...
class Implementation():
	...
	def add_person(self,firstname,secondname,person_type,wants_accommodation="N"):
		...
		if person_type.lower()=="fellow":
			...
			print("{} {} has been successfully added.".format(self.person.firstname,self.person.secondname))
	...
	def print_allocations(self,file=""):
		...
		#dictionary hold room and one person
		allocations={'room_name':'person_name'}
		#extract the list of room occupands
		for room_object  in self.all_rooms:
			print(room_object.room_name)
			if len(room_object.list_of_occupants)!=0:
				#loop through the list of occupants outputing the names
				for person_object in room_object.list_of_occupants:
					print(person_object.firstname,person_object.firstname)
